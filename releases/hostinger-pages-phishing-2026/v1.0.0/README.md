...
This release contains publication-safe aggregate observations, a complete defanged and role-classified domain inventory, contextual indicators, exact-hash summaries, descriptive hostname statistics, cross-brand context, a relationship graph, one synthetic loopback-only request-shape reconstruction and reproduction notes. It does not contain raw phishing HTML or JavaScript, response bodies, full task URLs, paths, query strings, recipient-derived fields, provider correspondence, API credentials, collection ledgers or executable malicious material.
...
- zero structurally valid ZIP archives from a fixed, hash-gated 25-path check; and
